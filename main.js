...
// Obtener los items
const itemsModelo = obtenerItemsSinc(); // Aquí obtienes los items de manera síncrona

//gestiona los  id o numeros de cadena a entero
items = itemsModelo.map((item) => ({
  ...item,
  id: parseInt(item.id, 10),
  CantidadSugerida: parseInt(item.CantidadSugerida, 10),
  CantidadMinina: parseInt(item.CantidadMinina, 10),
  Opcionales:
    typeof item.Opcionales === "string"
      ? JSON.parse(item.Opcionales || "{}")
      : item.Opcionales,
}));
...
const scriptURLCorreos =
  "https://script.google.com/macros/s/AKfycbw6I6TcDab7w3qiXrGPRpr4FbnUxdK6kqPsIWVi6cohcwBPlC02NVLH1TVa6ylp80UJzA/exec";
